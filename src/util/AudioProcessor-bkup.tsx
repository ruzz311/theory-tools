 ...
 * @returns 
 */
const createNote: (sampleRate: number, noteName: string) => INote = (sampleRate: number, noteName: string) => {
  const freq = noteToFrequency(noteName, { standard: TUNING_PITCH })
  return {
    noteName,
    freq,
 ...
 * @returns 
 */
export const createGuitarStrings: (sampleRate: number, tuningName?: string) => IGuitarStrings = (sampleRate, tuningName = 'standard') => {
  ...
  const strings = chosen.reduce((prev, noteName, i) => {
    prev[noteName] = createNote(sampleRate, noteName);
    return prev;
  }, {} as IGuitarStrings)
...
class AudioProcessor {
  ...
  dispatchAudioData(time) {
    ...
    let dominantFrequency = Math.log2(frequency / TUNING_PITCH);
    ...
    let octave = Math.floor(4 + ((9 + semitonesFromA4) / 12));

    // The note is 0 for A, all the way to 11 for G#.
    const noteNumber = (12 + (Math.round(semitonesFromA4) % 12)) % 12

    // The note is 0 for A, all the way to 11 for G#.
    const noteNameList = ['A', 'A#/Bb', 'B', 'C', 'C#/Db', 'D', 'D#/Eb', 'E', 'F', 'F#/Gb', 'G', 'G#/Ab'];
    ...
    const dataOutput: IAudioDataParams = {
      ...
      // The note is 0 for A, all the way to 11 for G#.
      note: noteNumber,
      noteName: noteNameList[noteNumber]
    };
